...
def create_user_database_path(username: str, app_type: str = "cli") -> str:
    """Create a user-specific database path."""
    # Sanitize username to be filesystem-safe
    safe_username = re.sub(r"[^\w\-_.]", "_", username.lower())
    return f"research_{safe_username}_{app_type}.db"
...
def validate_exa_api_key(api_key: str) -> bool:
    """Validate Exa API key format."""
    if not api_key:
        return False
    # Basic validation: has reasonable length (adjust as needed)
    return len(api_key) > 10


class Researcher:
    """A researcher class that manages Memori initialization and agent creation with multi-user support"""
    ...
    def __init__(
        ...
        # Validate API keys
        current_openai_key = openai_api_key or os.getenv("OPENAI_API_KEY")
        current_exa_key = exa_api_key or os.getenv("EXA_API_KEY")

        if not current_openai_key:
            raise ValueError("OpenAI API key is required")
        if not current_exa_key:
            raise ValueError("Exa API key is required")

        # Initialize Memori with user-specific database
    ...
    def _create_research_agent(self):
        ...
        agent = Agent(
            ...
            save_response_to_file=str(self.tmp.joinpath("{message}.md")),
        )
        return agent
...
# Legacy functions for backward compatibility - now using Researcher class
def create_research_agent(
    database_path: str = "research_memori.db",
    openai_api_key: str = None,
    exa_api_key: str = None,
):
    """Create a research agent using the Researcher class"""
    researcher = Researcher(database_path, openai_api_key, exa_api_key)
    return researcher.get_research_agent()
...
def create_memory_agent(
    ...
    """Create a memory agent using the Researcher class"""
    researcher = Researcher(database_path, openai_api_key)
    return researcher.get_memory_agent()
